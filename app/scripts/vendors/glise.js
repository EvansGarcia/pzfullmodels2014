...
(function ($) {
    $.glisse = function (element, options) {
        ...
        var preloadImgs = function preloadImgs(){
            var current, imageUrls = [], i, self = this;

            $('img[rel=''+group+'']').each(function(i,el){
                imageUrls.push($(this).attr(plugin.settings.dataName));
            });
            function loaded(current){
                cache.push(current);
            }
            for (i = 0; i < imageUrls.length; i += 1) {
                    current = jQuery('<img>').attr('src', imageUrls[i]);
                    current.load(loaded(imageUrls[i]));
            }
        };
